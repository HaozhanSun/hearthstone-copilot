...
<style>body{font:14px system-ui;margin:0;background:#17191d;color:#eee}main{display:grid;grid-template-columns:1fr 1fr;grid-template-rows:1fr 220px;height:100vh}section{padding:16px;border:1px solid #30343b;overflow:auto}#raw{grid-column:1/3;white-space:pre-wrap;font-family:monospace;color:#b8c0cc}pre{white-space:pre-wrap}button{padding:8px 12px}</style>
<main><section><h2>Screenshot</h2><img id=shot style="max-width:100%" alt="no screenshot supplied"></section><section><h2>Parsed state</h2><button onclick=discrepancy()>Report discrepancy</button><pre id=state>loading…</pre></section><section id=raw><h2>Raw log tail</h2><div id=tail></div></section></main>
<script>async function refresh(){let r=await fetch('/api/state');let s=await r.json();document.getElementById('state').textContent=JSON.stringify(s.point,null,2);document.getElementById('tail').textContent=s.raw_tail.join('');let img=document.getElementById('shot');img.src=s.screenshot?'/screenshot?ts='+Date.now():'';img.alt=s.screenshot||'no screenshot supplied';}async function discrepancy(){let note=prompt('What disagrees?');if(note)await fetch('/api/discrepancy',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({note})});}refresh();setInterval(refresh,1000);</script>"""
...
class _Handler(BaseHTTPRequestHandler):
    ...
    def do_POST(self) -> None:  # noqa: N802
        if urlparse(self.path).path != "/api/discrepancy":
...
def serve_debug_ui(state: DebugState | None = None, *, host: str = "127.0.0.1", port: int = 8765, fixture_dir: str | Path | None = None) -> ThreadingHTTPServer:
    ...
    handler = type("DebugHandler", (_Handler,), {"state": state, "fixture_dir": Path(fixture_dir) if fixture_dir else None})
